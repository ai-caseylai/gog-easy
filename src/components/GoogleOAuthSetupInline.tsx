  ...
  return (
    <div className="mt-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="text-sm font-semibold text-slate-900">Google OAuth 設定資料（同 `/setup`）</div>
        <button
          onClick={() => void load()}
      ...
      {state.status === 'loading' ? <div className="mt-4 h-24 animate-pulse rounded-2xl border border-slate-200 bg-white" /> : null}
      {state.status === 'error' ? <div className="mt-4 text-sm text-rose-700">{state.message}</div> : null}
